...
const role = computed(() => auth.primaryRole)

const isBibliotecario = computed(() =>
  role.value === 'ROLE_BIBLIOTECARIO'
)

const isAdminOrStudent = computed(() =>
  role.value === 'ROLE_ADMIN' || role.value === 'ROLE_ESTUDIANTE'
)
</script>
  ...
  <aside :class="sidebarClass" style="box-shadow: 4px 0 24px rgba(0,0,0,0.06)">
    ...
    <div class="flex items-center gap-3 px-4 h-16 border-b border-slate-100 shrink-0">
      <div class="w-8 h-8 rounded-lg bg-primary-600 flex items-center justify-center shrink-0">
        <svg v-if="isAdminOrStudent" class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor"
          stroke-width="2">
          <path d="M4 19.5A2.5 2.5 0 016.5 17H20" />
          <path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z" />
        </svg>
        <img v-else-if="isBibliotecario" :src="getUrl(auth.user?.biblioteca?.[0]?.logoUrl)"
          class="w-full h-full object-cover rounded-lg" alt="logo biblioteca" />
      </div>
