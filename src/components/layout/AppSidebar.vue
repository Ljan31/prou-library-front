...
<template>
  <aside :class="sidebarClass" style="box-shadow: 4px 0 24px rgba(0,0,0,0.06)">
    ...
    <div class="flex items-center gap-3 px-4 h-16 border-b border-slate-100 shrink-0">
      ...
      >
        <svg class="w-4 h-4 transition-transform duration-300" :class="ui.sidebarCollapsed ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <Transition name="fade">
      <div v-if="!ui.sidebarCollapsed" class="px-4 py-3 border-b border-slate-100 shrink-0">
        <div class="flex items-center gap-2.5">
          <div class="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 font-semibold text-xs shrink-0">
            {{ auth.displayName.charAt(0).toUpperCase() }}
          </div>
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-slate-800 truncate leading-tight">{{ auth.displayName }}</p>
            <span
              v-if="auth.primaryRole"
    ...
    <div class="border-t border-slate-100 p-2 shrink-0">
      <button
        ...
        :title="ui.sidebarCollapsed ? 'Cerrar Sesión' : undefined"
        @click="auth.logout()"
      >
