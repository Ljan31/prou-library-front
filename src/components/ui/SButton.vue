...
<template>
  <button :type="type" :disabled="disabled || loading"
    ...
    :class="[variants[variant], sizes[size], full ? 'w-full' : '']" @click="emit('click', $event)">
    <!-- Loading spinner -->
    <svg v-if="loading" class="animate-spin -ml-0.5 h-4 w-4 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none"
      viewBox="0 0 24 24">
